/******************************************************************************
  Trigger-Cycle Slave (arm side) v9 -- telemetry now carries glove data.

  WHAT CHANGED FROM v8:
    - BUG FIX: the console's "T 1" telemetry frame only ever contained
      "# owner sh el w1 w2 gr base" -- no mode/dPitch/dRoll. The ONE place
      that did print dPitch/dRoll (the human-readable "mode=.. dPitch=..
      dRoll=.." status line) was gated behind `!telemetryOn`, and the
      console turns telemetry on the instant it connects. Net result: the
      console could never show roll/pitch, under any conditions -- it
      wasn't a wiring/BT problem, the data just never left the arm in the
      stream the console reads. Fixed by appending lastMode/
      lastDeltaPitch/lastDeltaRoll to the end of the telemetry frame:
        # owner sh el w1 w2 gr base mode dPitch dRoll
      (appended, not inserted, so anything reading only the first 7
      fields still works unchanged.)

  WHAT CHANGED FROM v7 (v8's changes, unchanged from here down):
...
//   Wrist1 (Mode 3, pitch): hand down -> wrist moved UP, felt backwards -- flipped via
//                           INVERT_WRIST1=true, so now hand down -> wrist down, hand up -> wrist up
//   Wrist2 (Mode 3, roll):  hand up   -> "motion down"  (sounds inverted -- try INVERT_WRIST2=true if unwanted)
bool INVERT_SHOULDER = false, INVERT_ELBOW = false, INVERT_WRIST1 = true, INVERT_WRIST2 = false, INVERT_GRIPPER = false, INVERT_BASE = false;
...
void loop() {
  ...
  if (telemetryOn && millis() - lastTelemetry >= TELEMETRY_MS) {
    ...
    // v9: appended lastMode/lastDeltaPitch/lastDeltaRoll to the end of
    // the frame so the console can actually show what the glove is
    // sending. Appending (not inserting) keeps this backward-compatible
    // with anything that only reads the first 7 fields.
    Serial.print(F("# "));
    ...
    Serial.print(currentPosBase);     Serial.print(' ');
    Serial.print(lastMode);           Serial.print(' ');
    Serial.print(lastDeltaPitch, 1);  Serial.print(' ');
    Serial.println(lastDeltaRoll, 1);
  }
