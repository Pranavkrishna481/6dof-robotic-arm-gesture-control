/******************************************************************************
  Trigger-Cycle Master (glove side) v4 -- ONE MPU6050, ONE trigger, 5 modes,
  DIRECT (non-sequential) mode addressing via tap-count bursts.

  WHAT CHANGED FROM v3 (v2 in old comments below):
    - Added Mode 5: Base rotation. Roll (twisting your wrist, same gesture
      language as the Gripper mode) now drives the NEMA17 base stepper
      clockwise/counter-clockwise. Pitch is unused in this mode, same
      pattern as Gripper.
    - MODE_COUNT bumped 4 -> 5. Tap-count wrap now cycles 1..5 instead of
      1..4, so 5 taps lands on Base directly and 6 taps wraps to Shoulder.
  ...
  WHAT CHANGED FROM v1 (kept for history):
    ...
    - v2: taps are counted in a burst. You tap N times quickly (within
      TAP_WINDOW_MS of each other), then the code waits for the burst to
        ...
        5 taps -> Mode 5 (Base: roll drives base rotation CW/ACW)
      6+ taps wraps back around via modulo, so it's forgiving if you
...
// ---- Mode addressing ----
const int MODE_COUNT = 5; // 1=Shoulder, 2=Elbow, 3=Wrist, 4=Gripper, 5=Base
int mode = 1;
...
// ---- Non-blocking mode-confirm blink ----
// v3/v4 used blinkMode() with delay(120) x2 per blink -- for Mode 5 that's
// 1200ms where loop() never reaches the BT.println() below, which blows
// straight past the arm's 500ms STALE_MS and makes it falsely freeze on
// every mode switch. This version blinks in the background instead, so
// gesture packets keep flowing at the normal ~20Hz the whole time.
int blinkRemaining = 0;      // toggles left (2 per blink: on, then off)
bool blinkLedOn = false;
unsigned long blinkLastToggle = 0;
const unsigned long BLINK_HALF_MS = 120;

void requestBlink(int m) {
  blinkRemaining = m * 2;
  blinkLedOn = false;
...
void serviceBlink() {
  ...
  if (millis() - blinkLastToggle >= BLINK_HALF_MS) {
...
void setup() {
  ...
  recenter();
  Serial.println(F("Trigger-cycle master v4 ready. Mode 1 = Shoulder. Tap N times fast to jump to Mode N."));
  requestBlink(mode);
}

void loop() {
  // Serviced every iteration, non-blocking -- keeps ticking during sends/reads below.
  serviceBlink();
  ...
  if (tapCount > 0 && (millis() - lastTapTime) > TAP_WINDOW_MS) {
    ...
    requestBlink(mode); // confirms even if you re-tapped into the same mode
  }
